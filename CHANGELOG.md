...
- **`AudioScape:checkAssetHealth(assetIds, playerId?)`** — reports whether specific assets are still playable, and whether AudioScape can offer similarity for them. Statuses: `ok`, `moderated`, `deleted`, `delisted`, `private`, `unknown`. Up to 100 IDs per call, batched internally.
